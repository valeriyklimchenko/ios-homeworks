...
struct PostModel {
    ...
    static func makePostModel() -> [PostModel] {
        var  model = [PostModel]()
         ...
         model.append(PostModel(author: "Цератопсы",  description: "Инфраотряд цератопсов легко узнать по массивным головам, короткой шее, рогам и изогнутой морде, похожей на клюв. В переводе с латинского языка их название означает «рогатое лицо». Цератопсы были разных размеров, но большинство — ростом с быка или даже со слона. Они могли весить несколько тонн, так что далеко не каждый хищник рисковал сражаться с таким грозным противником. Самый яркий представитель этого инфраотряда—трицератопс.", image: UIImage(named: "Цератопсы")!, likes: 333, views: 333))
         model.append(PostModel(author: "Стегозавры",  description: "Стегозавров легко узнать по шипам на хвосте и пластинкам на спине. Ученые выдвигают разные предположения, зачем они были нужны динозаврам, но однозначно ответить на этот вопрос пока не смогли. Весь инфраотряд назван в честь самого известного представителя—стегозавра.", image: UIImage(named: "Стегозавры")!, likes: 444, views: 444))
         model.append(PostModel(author: "Орнитоподы",  description: "Первые птицетазовые динозавры были растительноядными. Они передвигались в основном на двух ногах, которые по строению очень напоминали птичьи. Поэтому их так и назвали - птиценогими, или орнитоподами. Со временем они развивались и становились крупнее. В результате появились все остальные группы птицетазовых динозавров.", image: UIImage(named: "Орнитоподы")!, likes: 333, views: 333))
